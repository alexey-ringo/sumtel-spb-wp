function VikaSlider(obj) {
  ...
  instance.move = function (k, speed, ind) {
    if (instance.animate == false) {
      ...
      left = left + ($('#carousel-' + instance.id + ' .item:eq(0)').width() + mr) * instance.offset;
      ...
      if (instance.stop_on_first == true) {
        leftm = (carousel_width - $('.carousel-left-' + instance.id).width()) / 2 + instance.left-5;
        if (left < leftm) left = leftm;
      }
      if (instance.stop_on_end == true) {
        leftx = $('.carousel-left-' + instance.id).width() + ((carousel_width - $('.carousel-left-' + instance.id).width()) / 2) - item_width * (items_length)+20; // or + mr
        leftx = -leftx;
